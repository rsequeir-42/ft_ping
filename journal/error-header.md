...
`error.h` tient en deux fonctions — pas une de plus — mais il tranche une question qui paraît anodine et ne l'est pas : *comment* `ft_ping` annonce qu'il a échoué. Une seule voix, un format figé, et un détail d'apostrophe qui décide de la conformité à l'octet. Petit fichier, donc, mais qui mérite qu'on déplie chacun de ses choix.

> Cet article suit `include/error.h` au plus près : à chaque évolution du fichier, il est mis à jour dans le même mouvement.

## Deux fonctions pour une seule voix
...
`error_report` écrit, sur la sortie d'erreur, la ligne « *programme* : *message* », puis enchaîne sur `error_try_hint`, qui ajoute la ligne de conseil habituelle. Un opérande d'hôte manquant produit donc exactement ces deux lignes :
...
Pourquoi *deux* fonctions, et pas une seule qui imprimerait tout ? Parce que la seconde ligne — le « Try … » — se retrouve derrière plusieurs erreurs, pas seulement l'hôte manquant. La factoriser dans `error_try_hint` évite de la réécrire à chaque point d'échec, et donc de risquer une variante (un mot, une apostrophe) qui trahirait l'uniformité. Une voix, une seule formulation.
