# Summary

[Avant-propos](avant-propos.md)

# Monter l'atelier

- [Construire avant d'écrire](build-system.md)
- [Répondre avant qu'on ne demande](preseed.md)
- [Une machine qu'on peut jeter](reproducible-vm.md)

# Poser les garde-fous

- [Se faire relire par des machines](quality-checks.md)
- [Le banc d'essai avant la pièce](test-harness.md)
- [Un gardien hors de ma portée](ci-pipeline.md)
- [Compter ses angles morts](coverage.md)

# Reconstruire ping

- [Un écho dans le réseau](how-ping-works.md)

# Le code, pièce par pièce

- [La carte avant le territoire](ft-ping-header.md)
- [Des interrupteurs et une promesse](options-header.md)
- [Une table et un aiguilleur](options-source.md)
- [Lire un nombre, et s'en méfier](options-values.md)
- [Le parseur passé au crible](test-options.md)
- [Au pied de la lettre](conformance.md)
- [Dire l'erreur d'une seule voix](error-header.md)
- [Imprimer sans guetter de réponse](error-source.md)
